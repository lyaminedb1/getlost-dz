...
def create_offer():
    ...
    oid = db_run("""INSERT INTO offers(agency_id,title,category,price,duration,region,description,
        image_url,images,itinerary,includes,available_dates,price_details,status) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)""", (
        ...
        json.dumps(d.get("dates", [])), d.get("priceDetails", ""), "pending"
    ))
    # Notify admins
    try:
...
def update_offer(oid):
    ...
    db_run("""UPDATE offers SET title=?,category=?,price=?,duration=?,region=?,
              description=?,image_url=?,images=?,itinerary=?,includes=?,available_dates=?,price_details=? WHERE id=?""", (
        ...
        json.dumps(d.get("itinerary", [])), json.dumps(d.get("includes", [])),
        json.dumps(d.get("dates", [])), d.get("priceDetails", offer.get("price_details", "")), oid
    ))
...
@bp.route("/offers/<int:oid>/archive", methods=["PATCH"])
@token_required
def toggle_archive(oid):
    u = g.user
    offer = db_query("SELECT * FROM offers WHERE id=?", (oid,), one=True)
    if not offer:
        return jsonify({"error": "Not found"}), 404
    if u["role"] == "agency" and offer["agency_id"] != u.get("agencyId"):
        return jsonify({"error": "Forbidden"}), 403
    new_val = not offer.get("archived", False)
    db_run("UPDATE offers SET archived=? WHERE id=?", (new_val, oid))
    return jsonify({"archived": new_val})


# ── Departures ────────────────────────────────────────────────────────────────

@bp.route("/offers/<int:oid>/departures", methods=["GET"])
def get_departures(oid):
    rows = db_query("""SELECT d.*,
        (SELECT COUNT(*) FROM bookings b WHERE b.offer_id=d.offer_id AND b.departure_id=d.id
         AND b.status IN ('confirmed','pre_reserved','completed')) as booked_count
        FROM offer_departures d WHERE d.offer_id=? ORDER BY d.departure_date ASC""", (oid,))
    return jsonify(rows)


@bp.route("/offers/<int:oid>/departures", methods=["POST"])
@token_required
def add_departure(oid):
    u = g.user
    offer = db_query("SELECT * FROM offers WHERE id=?", (oid,), one=True)
    if not offer:
        return jsonify({"error": "Not found"}), 404
    if u["role"] == "agency" and offer["agency_id"] != u.get("agencyId"):
        return jsonify({"error": "Forbidden"}), 403
    d = request.json or {}
    if not d.get("departure_date"):
        return jsonify({"error": "Date de depart requise"}), 400
    did = db_run(
        "INSERT INTO offer_departures(offer_id,departure_date,return_date,max_places,label) VALUES(?,?,?,?,?)",
        (oid, d["departure_date"], d.get("return_date", ""), int(d.get("max_places", 20)), d.get("label", ""))
    )
    return jsonify({"id": did}), 201


@bp.route("/departures/<int:did>", methods=["DELETE"])
@token_required
def delete_departure(did):
    dep = db_query("SELECT d.*, o.agency_id FROM offer_departures d JOIN offers o ON d.offer_id=o.id WHERE d.id=?", (did,), one=True)
    if not dep:
        return jsonify({"error": "Not found"}), 404
    if g.user["role"] == "agency" and dep["agency_id"] != g.user.get("agencyId"):
        return jsonify({"error": "Forbidden"}), 403
    db_run("DELETE FROM offer_departures WHERE id=?", (did,))
    return jsonify({"message": "Deleted"})
