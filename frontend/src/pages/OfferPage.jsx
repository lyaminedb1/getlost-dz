          ...
          <div style={{ position: 'sticky', top: 88 }}>
            <Card style={{ padding: 0, overflow: 'hidden' }}>
              ...
              <div style={{ padding: '20px' }}>
                ...
                {offer.agency_phone && (() => {
                  const cleanPhone = offer.agency_phone.replace(/\s/g, '').replace(/^\+/, '')
                  const waMsg = encodeURIComponent(`Bonjour ! Je suis intéressé(e) par votre offre "${offer.title}" sur Get Lost DZ.`)
                  return (
                    <a href={`https://wa.me/${cleanPhone}?text=${waMsg}`} target="_blank" rel="noopener noreferrer"
                      onClick={() => track('whatsapp_direct', { offer_id: offer.id })}
                      style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, width: '100%', padding: '12px 20px', marginTop: 10, borderRadius: 12, background: '#25D366', color: '#fff', fontWeight: 700, fontSize: 14, textDecoration: 'none', border: 'none', cursor: 'pointer', transition: 'opacity .2s' }}
